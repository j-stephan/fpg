...
__device__ __host__ auto Q_rsqrt(float number) -> float
{
...
auto body_body_interaction_h(float4 bi, float4 bj, float3 ai) -> float3
{
    ...
    auto dist_sixth = dist_sqr * dist_sqr * dist_sqr;
#ifdef SYCL
    auto inv_dist_cube = 1. / Q_rsqrt(dist_sixth);
#else
    #if __GNUC__ <= 7
    ...
    for(auto n = 2048ul; n <= 524288ul; n *= 2ul)
    {
        ...
        */
        ...
        for(auto block_size = warp_size; block_size <= 1024u; block_size *= 2)
        {
            ...
            {
                body_integration_d<<<tiles, block_size,
            ...
            CHECK(cudaMemcpy(positions_cmp.data(), copy_src, bytes,
